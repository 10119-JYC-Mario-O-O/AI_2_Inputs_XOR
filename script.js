(async () => {
    ...
    let fitParam = {
        ...
        callbacks: {
            onEpochEnd: (epoch, logs) => {
                document.getElementById('output').innerHTML = `epoch = ${epoch}, RMSE = ${Math.sqrt(logs.loss)}`;
            }
        }
    };
    ...
    // document.getElementById('output').innerText = model.predict(tf.tensor2d([[0, 0]], [1, 2])).dataSync();
